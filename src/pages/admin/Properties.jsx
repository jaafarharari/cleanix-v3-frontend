...
import { Plus, Home, MapPin, Edit2, Trash2, X, Check, Loader2 } from 'lucide-react';
import AdminNav from '@/components/AdminNav';

const emptyForm = { name: '', address: '', city: '', latitude: '', longitude: '', host_notes: '', checklist_template: [] };
...
export default function Properties() {
  ...
  const openEdit = (p) => { setForm({ ...p, checklist_template: p.checklist_template || [] }); setEditId(p.id); setShowForm(true); };
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {properties.map(p => (
              <div key={p.id} className="card-hover p-5 animate-slide-up">
                ...
                {p.checklist_template?.length > 0 && (
                  <p className="text-xs text-dark-500 mt-2">{p.checklist_template.length} checklist tasks</p>
                )}
              </div>
            ))}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input className="input-dark flex-1" placeholder="Add task..." value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTask()} />
